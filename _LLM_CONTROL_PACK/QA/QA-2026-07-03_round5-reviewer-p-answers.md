...
| **Symbologies** | **Code 128 + QR** in v1. DataMatrix phase 2. Standardize **BCS-printed** QR on `https://bluecollar-systems.com/p/<id>` (aligns Q-N10) while **reading** legacy Part Tracking Code 128 without re-printing. |
...
Omni-Box engine, inventory NL, and time NL now exist in Steel Logic (`lib/core/omni/`, Tools→Omni-Box). Barcode/Part Tracking scan (Q-P1) should route through the same intent router as the next slice.
