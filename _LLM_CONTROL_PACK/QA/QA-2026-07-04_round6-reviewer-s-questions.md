...
**Question:** Which row is still P0 for “importers nailed down” vs honestly P2 (app-side barcode/Part Tracking per R5-1)?
...
R5 defers barcode/Part Tracking to Steel Logic app. Importers already emit `ready_check`-shaped diagnostics in Python hosts.

**Question:** Should Round 6 add a **`import_contract_ready`** aggregate in `import_report.extra` — true only when `actual_text_entity_types`, `report_meta.build_stamp`, `scale_crosscheck`, and schema validation all pass — so the website Report Doctor and T-01 script can gate “start Part Tracking work” without reading six separate fields?
