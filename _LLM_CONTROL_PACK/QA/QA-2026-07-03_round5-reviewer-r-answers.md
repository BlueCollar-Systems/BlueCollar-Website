...
| **Symbology filter** | `BarcodeFormat.code128` + `BarcodeFormat.qrCode` **only** at `MobileScannerController` — ignore DataMatrix/EAN/UPC to cut false positives on shop clutter (Part Tracking + BCS QR cover v1 per R5-1). |
...
| `tier1/tags/code128_Part Tracking | Expected decode string `884422` (text, not image — CI cannot scan metal) |
