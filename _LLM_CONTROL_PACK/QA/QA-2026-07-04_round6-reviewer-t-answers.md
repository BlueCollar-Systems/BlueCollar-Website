...
**P2 (app/advanced):** Part Tracking/barcode (R5-1), `parts_bootstrap` (R5-2), `source_provenance` emitter (R5-3), public `/p/` URLs (R5-6).
...
**Answer: Yes as P2 stub; not Round 6 blocker.** Compute client-side in Report Doctor first: `ready = actual_text_entity_types present && build_stamp present && no open_failure`. Avoid new schema until three hosts emit stamps. Part Tracking work starts when **P0 importer rows** in S5 are green — not when this boolean exists.
