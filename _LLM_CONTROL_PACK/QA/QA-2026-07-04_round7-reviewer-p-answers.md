...
**Answer:** Sidecar **shipped** in SU v3.7.79. App stub reads summary fields today; full reverse-tag highlight requires app to load sidecar objects — **deferred** to Part Tracking loop (R7-11). Summary-only was R6-8 partial; R7-3 closes the SU emitter gap.

## P→O-4 — `import_contract_ready`

**Answer:** **Advisory only** until T-01 green. Report Doctor may surface it prominently, but Part Tracking work may proceed on P0 contract rows without treating `ready: true` as human visual sign-off.
...
**Answer:** Field-test script for shop flows (scan → shape lookup, import_report drop folder), Android/iOS `steellogic://` manifest entries, and Part Tracking tag-sheet UI remain P1. v1.0.10 closes R5-1/R5-2 stub/R5-6.
...
| R6-10 Part Tracking | App scan **SHIPPED**; full loop **DEFERRED** |
